...
# ── Лимит стоимости позиции (единый источник правды, см. broker_finam) ──
# Защита от маржинальных требований: при превышении лотность урезается.
...
# ── Фьючерсы MOEX FORTS ─────────────────────────────────
# Отключены: маржинальная логика (ГО) не реализована — фьючерсы жёстко
# блокируются в broker_finam.execute_signal (No enough coverage).
# Чтобы вернуть — добавь тикеры обратно и реализуй расчёт ГО в calc_lots.
# Ранее сканировались: MXU6, GDU6.
MOEX_FUTURES = []
